...
def work(cap, model_type, optimize, z, pca):
    global first_run
    global pressedFlag
    ret, frame = cap.read()
    ...
    if ret:
        ...
        # original_image_rgb = cv2.resize(original_image_rgb, (new_shape_w//5,new_shape_h//5))
        ...
        dets, frameYOLO = hands_yolo(frameYOLO, net)

        xy = []
        depth_out = depth.copy()
        for i, det in enumerate(dets):
            det[:, :4] = scale_coords(frameYOLO.shape[2:], det[:, :4], frameOrig.shape).round()
            for *xyxy, conf, cls in reversed(det):
                x,y = plot_one_box(xyxy, depth_out, label=None, color=(255,0,0), line_thickness=1)
                xy += [[x, y]]
                # xy += [(det[:,2] - det[:,0]) // 2, (det[:,3] - det[:, 1]) // 2]
        # centers = np.array([dets[:,0] + (dets[:,2]-dets[:,0]) // 2, dets[:,1] + (dets[:,3]-dets[:,1]) // 2])
        ...
        for point in xy:
            ...
            cv2.putText(depth_out, str(prediction[point[1], point[0]]), point-50, font, 
                   0.5, (0,0,255), 1, cv2.LINE_AA)
        # clusters = cluster(dets[:, :2].astype(np.float32))
        ...
        # for i in range(dets.shape[0]):
            # cv2.rectangle(out, (dets[i][0], dets[i][1]), (dets[i][2], dets[i][3]), [0, 0, 255], 3)
            # center_y = dets[i][0] + (dets[i][2]-dets[i][0]) // 2
            # center_x = dets[i][1] + (dets[i][3]-dets[i][1]) // 2
            # if clusters[i] == 0:
            ...
            # cv2.circle(frame, centeroidA, 5, (0, 0, 255), 5)
                # if clusters[i] == 1:
            # if 1 in clusters:
                # cv2.circle(frame, centeroidB, 5, (255, 0, 0), 5)

        if pressedFlag:
            if len(xy) > 0:
                print('A1')
                first_run = False
                gen_img, z = gen_images.generate_images(gen_model, [xy[0][1], xy[0][0], prediction[xy[0][1], xy[0][0]]], z, 3, 1, 'random', './', (0, 0), 0, pca)
                return depth_out, gen_img, z
            elif len(xy) == 0 and not first_run:
                print('B1')
                gen_img, z = gen_images.generate_images(gen_model, None, z, 3, 1, 'random', './', (0, 0), 0, pca)
                return depth_out, gen_img, z
            elif len(xy) == 0 and first_run:
                print('C1')
                return depth_out, np.zeros((512, 512), dtype = np.uint8), z
        else:
            if len(xy) > 0:
                print('A2')
                first_run = False
...
def run(input_path, output_path, model_path, model_type="dpt_beit_large_512", optimize=False, side=False, height=None,
    ...
    z = np.random.RandomState(100).randn(1, 512)
    ...
    while cap.isOpened(): #and capR.isOpened():
        ...
        out, gen_img, z_pca = work(cap, model_type, optimize, z_pca, pca)

        # out = cv2.resize(out, (625, 625))
        out = draw_grid(out, (16, 16), color=(0,0,255))
